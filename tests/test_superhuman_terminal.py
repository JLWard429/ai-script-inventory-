...
class TestSuperhumanTerminal:
    ...
    def test_latest_file_detection(self):
        ...
        for prompt, expected_intent in latest_prompts:
            ...
            assert 'latest' in intent.parameters.get('scope', '') or \
                   'recent' in intent.parameters.get('scope', '')
    ...
    def test_find_latest_file(self, mock_listdir, mock_isdir, mock_getmtime, mock_isfile, mock_exists):
        """Test latest file detection functionality."""
        # Mock file system
        mock_exists.return_value = True
        mock_isdir.return_value = True
        mock_isfile.return_value = True
        mock_getmtime.return_value = 1000  # Return same time for all - will fall back to alphabetical
...
class TestIntentParameterExtraction:
    ...
    def test_directory_extraction(self):
        ...
        for prompt, expected_dir in prompts_with_dirs:
            ...
            dir_found = (intent.target == expected_dir or 
                        intent.parameters.get('directory') == expected_dir or
                        expected_dir in intent.parameters.get('directory', '') or
                        expected_dir in prompt.lower())  # At least the word should be in the prompt
            assert dir_found, f"Directory '{expected_dir}' not found in intent for prompt '{prompt}'"
    ...
    def test_file_type_extraction(self):
        ...
        for prompt, expected_type in type_prompts:
            ...
            if file_type:
                ...
                assert expected_type.lower() in prompt.lower() or 'python' in prompt.lower()

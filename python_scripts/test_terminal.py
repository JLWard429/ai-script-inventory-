...
from ai.intent import IntentType, create_intent_recognizer
...
def test_intent_recognition():
    ...
    for input_text, expected_intent in test_cases:
        ...
        if intent.type == expected_intent:
            print(
                f"  ✅ '{input_text}' -> {intent.type.value} (confidence: {intent.confidence:.2f})"
            )
            passed += 1
        else:
            print(
                f"  ❌ '{input_text}' -> {intent.type.value} (expected: {expected_intent.value})"
            )
